...
func _refresh_display() -> void:
	funds_label.text = "Fonds : %.2f" % UserSession.account_funds
	shields_label.text = "Boucliers : %d" % UserSession.shields_owned
...
func _on_buy_pressed() -> void:
	...
	if qty <= 0:
		status_label.text = "Entrez une quantité valide."
		return
	_pending_action = "purchase"
	status_label.text = "Achat de %d bouclier(s)..." % qty
...
func _on_api_response(response_code: int, body: Dictionary) -> void:
	...
	if response_code == 200:
		...
		_refresh_display()
		status_label.text = "Achat réussi !"
		status_label.modulate = Color(0.4, 1.0, 0.4)
	else:
		status_label.text = body.get("detail", "Échec de l'achat.")
